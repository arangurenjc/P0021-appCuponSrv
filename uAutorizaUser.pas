...
uses
  ...
  Vcl.Menus, System.ImageList, Vcl.ImgList, Vcl.Buttons, sSpeedButton;
...
begin
  ...
  StringGrid1.cells[colGrid + 3, rowgrid] := PERMISO_ON;
...
begin
  ...
  StringGrid1.cells[colGrid + 3, rowgrid] := PERMISO_OFF;
end;
...
procedure TfAutorizaUser.sSBGuardarClick(Sender: TObject);
begin
  updateUser;
end;
...
begin
  ...
  begin
    ...
    begin
      Brush.Color :=  udm.arrayColorAzul; //clNavy;      // le ponemos azul de fondo
    ...
    begin
      ...
      begin
        Brush.Color := uDM.arrayColorVerde; //RGB(88,166,56); //clRed;     // fondo rojo
        Font.Color  := clWhite;    // fuente blanca
        Font.Style  := [fsBold];   // y negrita
      end
      else
    ...
    begin
      ...
      begin
        if StringGrid1.Cells[j, i] = '' then
          arrayPermiso := arrayPermiso + '0'
          else
          arrayPermiso := arrayPermiso + '1'
      end;

      ParamByName('PermisosActivos').AsString := arrayPermiso;
